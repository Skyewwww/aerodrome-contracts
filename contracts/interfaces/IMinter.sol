...
interface IMinter {
    ...
    /// @notice Growth rate of second Bootstrap Phase emissions as percentage of `MAX_BPS` in first 9 weeks
    function SECOND_WEEKLY_GROWTH() external view returns (uint256);

    /// @notice Growth rate of first Bootstrap Phase emissions as percentage of `MAX_BPS` in first 9 weeks
    function FIRST_WEEKLY_GROWTH() external view returns (uint256);
